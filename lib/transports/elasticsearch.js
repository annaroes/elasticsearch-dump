...
elasticsearch.prototype.get = function(limit, offset, callback){
    ...
    if(type === 'data'){
        ...
        self.getAnalyzer(limit, offset, callback);
    }else{
        callback(new Error('unknown type option'), null);
    }
};

// TODO only works if index is set -- should be set anyway for shard settings etc. to work but never asserted
elasticsearch.prototype.getAnalyzer = function(limit, offset, callback) {
    var self = this;
    if (self.gotAnalyzer === true) {
        callback(null, []);
    } else {
        var url = self.baseUrl + '/_settings';
        var indexParts = self.baseUrl.split('/');
        var index = indexParts[(indexParts.length - 1)];
        request.get(url, function(err, response){
            self.gotAnalyzer = true;
            var payload = [];
            if (!err) {
                var returnBody = null;
                try{
                    var body = jsonParser.parse(response.body);
                    var analysis = body[index].settings.index.analysis;
                    if (analysis) {
                        returnBody = {};
                        returnBody[index] = {};
                        returnBody[index].analysis = analysis;
                    }
                    response = payload.push(JSON.stringify(returnBody));
                }catch(e){
                    callback(e, payload);
                }
            }
...
// TODO only works if analyzer is set before index is created
elasticsearch.prototype.setAnalyzer = function(data, limit, offset, callback) {
    ...
    if (self.haveSetAnalyzer === true) {
        ...
        for (var index in data) {
            var analysis = data[index];
            var requestBody = {};
            requestBody['settings'] = {};
            requestBody['settings']['analysis'] = analysis;
